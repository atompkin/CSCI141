...
public class GameBoard
{
    ...
    public int findBestMoveColumn(PieceType type)
    {
    	...
    	for(int i = 0; i < myNumColumns; i++)
    	{
    				...
    				countDiagonalLengthIfPiecePlacedWin(i,type) == i)
    		{
    			if(this.isColumnFull(i)==true)
    	    	{
    	    		...
    	    		int current = i;
    	    		while(current == i) {
    	    			current = random.nextInt(myNumColumns);
    	    		}
    	    		return current;
    	    	}
    			return i;
    	...
    	int random = rand.nextInt((this.getNumColumns()-1) - 0 + 1);
    	return random;
    }
    ...
    /**
     * I used what I created in my model
     * @author Malak Patel
    ...
    public boolean isBoardFull() 
    {
  	  ...
  	  {
  		  for(int j = 0; j <=(myNumColumns-1); j++)
  		  {
  			  if(myBoard[i][j]==null)
  			  {
  				  return false;
